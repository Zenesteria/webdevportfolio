...
export const fetchContentData: (slug:string) => Promise<PageProps> = async (slug) => {
  const postRes = await fetch(`${serverUrl}/api/blog/getPost`, {next:{revalidate:3600}, method:'POST', body:JSON.stringify({slug})});
...
export default async function Post({ params }: { params: { slug: string } }) {
  const { post } = await fetchContentData(params.slug);
